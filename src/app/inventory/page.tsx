    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-industrial tracking-tight">Industries</h1>
                    <p className="text-industrial/60 mt-1">Manage items and specifications categorized by industry.</p>
                </div>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <SearchBar placeholder="Search industries or their items..." />
                {query ? (
                    <p className="text-sm text-industrial/50">
                        {filteredClients.length} industry result{filteredClients.length === 1 ? '' : 's'} and {matchingItems.length} item result{matchingItems.length === 1 ? '' : 's'} for &quot;{rawQuery.trim()}&quot;
                    </p>
                ) : null}
            </div>
            ...
            {clients.length === 0 ? (
                <div className="text-center py-20 border-2 border-dashed border-kraft-dark/20 rounded-lg bg-kraft/10">
                    <p className="text-industrial/40 mb-4">No industries found.</p>
                    <Link href="/inventory/new-client">
                        <Button variant="secondary">Add your first industry</Button>
                    </Link>
                </div>
            ) : query && filteredClients.length === 0 && matchingItems.length === 0 ? (
                <div className="text-center py-20 border-2 border-dashed border-kraft-dark/20 rounded-lg bg-kraft/10">
                    <p className="text-industrial/40 mb-4">No industries or items match &quot;{rawQuery.trim()}&quot;.</p>
                    <Link href="/inventory">
                ...
                <div className="space-y-8">
                    <section className="space-y-4">
                        <div className="flex items-center justify-between gap-4">
                            <h2 className="text-lg font-bold text-industrial">Industries</h2>
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-industrial/40 uppercase tracking-wider">
                                                        {query && clientMatchCount > 0 ? `${clientMatchCount} matching item${clientMatchCount === 1 ? '' : 's'}` : 'Industry Category'}
                                                    </p>
                                                </div>
                                            </div>
                                    ...
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="bg-industrial text-kraft-lighter">
                                                <th className="px-6 py-4 font-semibold uppercase text-xs tracking-wider">Industry</th>
